...
int main(int argc, char** argv){
    ...
    while(nh.ok()){
        __u8 data[14];
        __u8 reg = 0x3b;
        i2c_smbus_read_block_data(file, reg, data);
	...
	imu.header.stamp = ros::Time::now();        
        ...
        int sonar_data = (int) (i2c_read_byte(file) << 8) | i2c_read_byte(file);
        std_msgs::Int16 left_count;
        left_count.data = (int) (i2c_read_byte(file) << 8) | i2c_read_byte(file);
        std_msgs::Int16 right_count;
        right_count.data = (int) (i2c_read_byte(file) << 8) | i2c_read_byte(file);    
